...
- [ ] `oss-wee` リポジトリ内に `wall-of-hands/` ディレクトリを新設し、本仕様書を配置
- [ ] 上記JSONスキーマをJSON Schema形式（`.schema.json`）として正式化
- [ ] WAVE→GRIP→COLLAPSEの状態遷移を検証する最小限のPythonスクリプト（`validate_grip.py`）を試作
- [ ] Witness機能の最小実装として、GitHub Issue/PRのコメント機能をWitness署名の代替UIとして流用する案の検討（新しいアプリを作らず、既存のGitHubの「壁」をそのままWallとして使う）
